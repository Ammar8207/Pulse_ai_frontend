...
function RecommendationCard({ title, description, icon: Icon }) {
  return (
    <div className="flex flex-col gap-3">
      <div className="grid h-9 w-9 shrink-0 place-items-center rounded-lg border border-white/[0.06] bg-white/[0.02] text-slate-300">
        <Icon size={16} aria-hidden="true" />
      </div>
      <div>
        <h4 className="text-[14px] font-bold text-white tracking-tight">{title}</h4>
...
export default function BusinessInsights() {
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      ...
      try {
        // Fetch KPI summary and rule-based insights in parallel — independent endpoints
        const [summaryRes, insightsRes] = await Promise.all([
          apiClient.get('/dashboard-summary'),
          apiClient.get('/business-insights'),
        ]);
        ...
        if (active) {
          setError(
            requestError?.response?.data?.detail || requestError?.message || 'Unable to load business insights.',
          );
          setSummary({});
          setInsights({ recommendations: [], actions: [] });
        }
      } finally {
        if (active) {
          setLoading(false);
  ...
  // Backend: /dashboard-summary returns flat fields
  const metrics = useMemo(() => {
    if (!summary) return [];
  ...
  // Backend: /business-insights returns { recommendations: [{title, description}], actions: [string] }
  const recommendations = useMemo(() => {
    const source = Array.isArray(insights?.recommendations) ? insights.recommendations : [];
    return source.map(normalizeRecommendation);
  }, [insights]);

  const actions = useMemo(() => {
    return Array.isArray(insights?.actions) ? insights.actions : [];
  }, [insights]);
    ...
    <section className="grid gap-6">
      <div className="app-panel p-6 sm:p-8">
        <div className="max-w-3xl">
          ...
          <h2 className="mt-2 text-3xl font-semibold text-white">Strategic Retention & Account Health Playbook</h2>
          <p className="mt-3 max-w-2xl text-sm font-light leading-6 text-slate-300 sm:text-base">
            Real-time account health metrics and rule-based retention recommendations, derived directly
            from live churn predictions and business data.
          </p>
        </div>
        ...
        {!loading && !error && (
          <div className="mt-8 border-t border-white/[0.06] pt-6 grid gap-6 sm:grid-cols-3">
            {metrics.map((metric, index) => (
              <div
                key={metric.title}
                className={['flex items-center justify-between gap-4', index > 0 ? 'sm:border-l sm:border-white/[0.06] sm:pl-6' : ''].join(' ')}
              >
                <div>
                  <p className="type-label text-slate-500">{metric.title}</p>
                  <h3 className="mt-1 text-2xl font-bold tracking-tight text-white">{metric.value}</h3>
        ...
        <div className="grid gap-6 xl:grid-cols-[1.1fr_0.9fr]">
          <section className="app-panel p-6">
            <div className="mb-5 flex items-center gap-3">
              ...
              <div>
                <p className="type-label">Rule-Based Playbooks</p>
                <h3 className="text-xl font-semibold text-white">Targeted Mitigation Playbooks</h3>
              </div>
            </div>
          ...
          <section className="app-panel p-6">
            ...
            {actions.length ? (
              ...
              <p className="text-sm text-slate-400">No actions available right now.</p>
            )}
          </section>
        </div>
      )}
